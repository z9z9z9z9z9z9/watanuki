    ...
    <>
      <div className="banner min-h-[700px] relative w-full bg-[#262525] pt-10 md:pt-20">
        <div className="backdrop-img bg-backGround w-full h-full absolute top-0 left-0 overflow-hidden opacity-[.1]">
          <img src={data.poster} alt={data.title} className="object-cover object-center h-full w-full" loading="lazy" />
        </div>
        ...
        <div className="content max-w-[1200px] w-full mx-auto flex flex-col items-start md:flex-row gap-2 mb-2 relative px-2">
          ...
          <div className="right mt-3 w-full flex flex-col gap-2">
            ...
            <h1 className="title text-lg md:text-4xl font-extrabold">{data.title}</h1>
            <div className="alternative gray text-lg font-bold">{data.alternativeTitle}</div>
            <div className="alternative gray text-lg font-bold">{data.japanese}</div>
            <div className="sounds flex items-center gap-2 my-2">
              ...
              />
              <span className="h-1 w-1 rounded-full bg-primary"></span>
              <span className="type text-[#ccc] text-sm font-bold">{data.type}</span>
              <span className="h-1 w-1 rounded-full bg-primary"></span>
              <span className="duration text-[#ccc] text-sm font-bold">{data.duration}</span>
            </div>
            ...
            {data.synopsis && (
              <div className="overview">
                <p className={`${showFull ? 'line-clamp-none' : 'line-clamp-3'} text-balance text-gray-300`}>
                  {data.synopsis}
                </p>
                <span onClick={() => setShowFull(!showFull)} className="text-sm cursor-pointer font-extrabold">
                  {showFull ? ' - LESS' : ' - MORE'}
                </span>
              </div>
            )}
            ...
            {data.studios && (
              <>
                <div className="studio">
                  <span>Studio : </span>
                  <Link to={`/producer/${data.studios.toLowerCase().replace(' ', '-')}`}>
              ...
              {data.producers && (
                <ul className="flex flex-wrap gap-2">
                  {data.producers.map((producer, index) => (
                    <Link to={`/producer/${producer}`} key={producer} className="hover:opacity-[0.7]">
                      <li style={{ color: colors[index % colors.length] }}>{producer}</li>
                    </Link>
                  ))}
                </ul>
              )}
