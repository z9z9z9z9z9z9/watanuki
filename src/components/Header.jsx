...
const Header = () => {
  ...
  const { data, isLoading, isError, error } = useApi(debouncedValue.length > 2 ? `/suggestion?keyword=${debouncedValue}` : null)
      ...
      <div className="z-10 absolute bg-backGround w-full  py-2">
        <div className="flex gap-2 px-5 md:px-10 md:gap-5 justify-between items-center">
          ...
          <div className="right justify-end lg:basis-[40%] flex gap-2 md:gap-5 items-center">
            <button className="text-xl" onClick={() => setShowSearchBar(!showSearchBar)}>
              {showSearchBar ? <FaXmark /> : <FaSearch />}
            </button>
          </div>
        </div>
        <form action={`/search?keyword=${value}`} onSubmit={handleSubmit} className={`search mt-2 px-4 relative items-center w-full ${showSearchBar ? 'flex' : 'hidden'}`}>
          <input value={value} onChange={changeInput} placeholder="search anime" type="text" className="header-search w-full bg-[#FBF8EF] px-2 text-lg text-black py-2 rounded-md" />
        ...
        <div className={`${showSearchBar ? 'flex flex-col mt-4' : 'hidden'}`}>
          {isLoading ? (
            ...
            <>
              {data?.data?.map((item) => (
                <div onClick={() => navigateToAnimePage(item.id)} className="flex w-full justify-start items-start hover:bg-lightBg px-3 py-5 gap-4" key={item.id}>
                  <div className="poster shrink-0 pb-14 relative w-10">
                    <img className="h-full w-full inset-0 absolute object-cover object-center" src={item.poster} alt={item.title} />
                  </div>
                  <div className="info">
                    <h4 className="title line-clamp-2">{item.title}</h4>
                    <h6 className="gray text-sm line-clamp-1">{item.alternativeTitle}</h6>
            ...
            <>{value.length > 2 && <h1 className="text-center text-lg text-primary">anime not found :(</h1>}</>
          )}
        </div>
      </div>
    </div>
