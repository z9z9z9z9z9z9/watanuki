...
const SoundsInfo = ({ episodes }) => {
  return (
    <div className="flex gap-0.5 font-extrabold">
      <div className="item bg-green text-sm px-1 text-black rounded-sm rounded-tr-none rounded-br-none">
        <FaClosedCaptioning />
        <span className="text-sm font-bold">{episodes.sub}</span>
      </div>

      <div className="item bg-blue px-1 text-[12px] text-black">
        <FaMicrophone />
        <span className="text-sm font-bold">{episodes.dub}</span>
      </div>

      <div className="item px-1 rounded-sm rounded-tl-none rounded-bl-none text-sm bg-gray">
        {episodes.eps}
      </div>
    </div>
  );
